...
public class BestStrategy implements ComputerPlayerStrategy {
    ...
    public BestStrategy(Board board) {
        ...
        List<ShipLocation.ShipOrientation> allOrientations = allPossible(fleet, this.fleetMap);
        this.allLocations = new ArrayList<ShipLocation>();
        for (ShipLocation.ShipOrientation so : allOrientations) {
    ...
    public void resultOfMove(Outcome outcome) {
        if (!outcome.hit()) {
            ...
            if (s != null) {
                ...
                ShipLocation.ShipOrientation toRemove = createShipOrientation(s, this.fleetMap.get(s.getName()));
                this.allLocations.removeIf(sl -> sl.orientation.getName().equals(toRemove.getName()));
            }
        }
    }

    private ShipLocation.ShipOrientation createShipOrientation(Ship ship, int shipNumber) {
        ...
        return new ShipLocation.ShipOrientation(ship.getName() + shipNumber, pattern);
    }

    private Set<ShipLocation.ShipOrientation> createAllOrientations(Ship ship, int shipNumber) {
        HashSet<ShipLocation.ShipOrientation> set = new HashSet<>();
        for (int i = 0; i < 4; i++) {
    ...
    private List<ShipLocation.ShipOrientation> allPossible(Fleet fleet, TreeMap<String, Integer> fleetMap) {
        List<ShipLocation.ShipOrientation> list = new ArrayList<>();
